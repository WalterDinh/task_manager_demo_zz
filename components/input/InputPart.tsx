...
export const inputSizeVariant: SizeVariantSpreadFunction<any> = (
  ...
  const radiusToken =
    extras.tokens.radius[val as keyof typeof extras.tokens.radius] ??
    extras.tokens.radius["$true"];
...
export const InputIconFrame = styled(View, {
  ...
  variants: {
    size: {
      "...size": (val, { tokens }) => {
        return {
          paddingHorizontal: tokens.space[val as keyof typeof tokens.space],
        };
      },
    },
  } as const,
...
const InputXGroup = styled(XGroup, {
  ...
  variants: {
    size: {
      "...size": (val, { tokens }) => {
        const radiusToken =
          tokens.radius[val as keyof typeof tokens.radius] ??
          tokens.radius["$true"];
        return {
